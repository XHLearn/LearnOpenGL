...
const char *fShaderSource[2] = {"#version 330 core\n"
                                "out vec4 FragColor;\n"
                                "void main()\n"
                                "{\n"
...
int main()
{
    ...
    // 编译片段着色器，两个
    unsigned int fShader[2];
    unsigned int shaderProgram[2];
    for (unsigned int i = 0; i < 2; i++)
    {
        fShader[i] = glCreateShader(GL_FRAGMENT_SHADER);
        glShaderSource(fShader[i], 1, &fShaderSource[i], nullptr);
        glCompileShader(fShader[i]);

        // 着色器程序-多个着色器合并之后并最终链接完成的版本
        shaderProgram[i] = glCreateProgram();
        glAttachShader(shaderProgram[i], vShader);
        glAttachShader(shaderProgram[i], fShader[i]);
